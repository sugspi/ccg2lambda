...
plain_dir=${sentences_fname%/*} #"en_plain" # tokenized sentences.
...
mkdir -p $parsed_dir $results_dir #$plain_dir
...
function parse_depccg() {
    # Parse using depccg.
    base_fname=$1
    lemmatize ${plain_dir}/${base_fname} | \
    ${candc_dir}/bin/pos \
        --model ${candc_dir}/models/pos \
        --ifmt "%w|%l \n" \
        --ofmt "%w|%l|%p \n" \
